...
/**
 * The main AttributeSwap screen, opened by the configured key (default: L).
 *
 * Layout:
...
public class AttributeSwapScreen extends Screen {
    ...
    protected void init() {
        ...
        // Left panel buttons
        btnAdd    = addDrawableChild(ButtonWidget.builder(Text.translatable("gui.attributeswap.add"),      btn -> onAddProfile())
                .dimensions(PAD, bY, 20, ROW_H).build());
        btnRename = addDrawableChild(ButtonWidget.builder(Text.translatable("gui.attributeswap.rename"), btn -> onRenameProfile())
                .dimensions(PAD + 22, bY, 56, ROW_H).build());
        btnDelete = addDrawableChild(ButtonWidget.builder(Text.translatable("gui.attributeswap.delete"), btn -> onDeleteProfile())
                .dimensions(PAD + 80, bY, 56, ROW_H).build());

        // Right panel toolbar
        int rX = LEFT_W + PAD;
        btnExport    = addDrawableChild(ButtonWidget.builder(Text.translatable("gui.attributeswap.export"),     btn -> onExport())
                .dimensions(rX, bY, 52, ROW_H).build());
        btnImport    = addDrawableChild(ButtonWidget.builder(Text.translatable("gui.attributeswap.import"),     btn -> onImport())
                .dimensions(rX + 54, bY, 52, ROW_H).build());
        btnNewFolder = addDrawableChild(ButtonWidget.builder(Text.translatable("gui.attributeswap.add_folder"),   btn -> onAddFolder())
    ...
    private void drawLeftPanel(DrawContext ctx, int mx, int my) {
        ...
        ctx.drawCenteredTextWithShadow(textRenderer, Text.translatable("gui.attributeswap.profiles").getString(), LEFT_W / 2, PAD + 2, C_TEXT);
    ...
    private void drawRightPanel(DrawContext ctx, int mx, int my) {
        ...
        if (selectedProfile == null) {
            ctx.drawCenteredTextWithShadow(textRenderer, Text.translatable("gui.attributeswap.select_profile").getString(),
        ...
        String title = String.format(Text.translatable("gui.attributeswap.active_count").getString(),
        ...
        for (ProfileFolder folder : selectedProfile.getFolders()) {
            ...
            if (itemY + ROW_H > yTop && itemY < yBot) {
                ...
                ctx.drawTextWithShadow(textRenderer, Text.translatable("gui.attributeswap.add_binding").getString(), lx + 2, itemY + 6, C_BLUE);
            }
            itemY += ROW_H;
        }
    }
    ...
    private int drawBindingRow(DrawContext ctx, int mx, int my,
        ...
        if (itemY + ROW_H > yTop && itemY < yBot) {
            ...
            ctx.drawTextWithShadow(textRenderer, "[" + Text.translatable("gui.attributeswap.edit").getString() + "]",   lx + rowW - 90, itemY + 6, C_BLUE);
            ctx.drawTextWithShadow(textRenderer, "[" + Text.translatable("gui.attributeswap.delete").getString() + "]", lx + rowW - 48, itemY + 6, C_RED);
        }
        return itemY + ROW_H;
    }
    ...
    private boolean clickRightPanel(int mx, int my) {
        ...
        for (ProfileFolder folder : selectedProfile.getFolders()) {
            ...
            // "Add Binding" row
            if (my >= itemY && my < itemY + ROW_H) {
                Binding nb = new Binding(Text.translatable("gui.attributeswap.new_binding").getString());
    ...
    private void onRenameProfile() {
        ...
        client.setScreen(new ProfileScreen(this, String.format(Text.translatable("gui.attributeswap.prompt.rename").getString(), target.getName()), name -> {
    ...
    private void onAddFolder() {
        if (selectedProfile == null) return;
        client.setScreen(new ProfileScreen(this, Text.translatable("gui.attributeswap.prompt.folder_name").getString(), name -> {
            if (name != null) {
